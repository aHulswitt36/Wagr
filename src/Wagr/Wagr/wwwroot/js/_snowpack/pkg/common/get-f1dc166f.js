import { p as createCommonjsModule, q as require$$0$1, s as commonjsGlobal, t as inherits_browser, u as getDefaultExportFromNamespaceIfNotNamed, v as Buffer, w as loglevel, d as _defineProperty, x as safeatob, y as get$1, z as randomId, D as keccak, E as base64url, F as jsonToBase64, G as getRpcPromiseCallback, o as lodash_merge, H as setupMultiplex, P as PostMessageStream, S as SafeEventEmitter, I as createStreamMiddleware, J as pump_1, K as JRPCEngine, L as createIdRemapMiddleware, M as getPrototypeOf, N as getDefaultExportFromCjs } from './base.esm-8d0d3561.js';
...
var nodeCrypto = /*@__PURE__*/getDefaultExportFromNamespaceIfNotNamed(_polyfillNode_crypto$1);
...
class OpenLoginStore {
  ...
  static getInstance(storeNamespace) {
    ...
    if (!this.instance) {
      ...
      const finalStoreKey = storeNamespace ? "".concat(storeKey, "_").concat(storeNamespace) : storeKey;
...
class Provider extends SafeEventEmitter {
  ...
  setupStream() {
    ...
    pump_1(JRPCConnection.stream, this.mux.createStream("jrpc"), JRPCConnection.stream, error => {
...
class OpenLogin {
  ...
  async getWhitelist() {
    try {
      ...
      const res = await get$1(url.href);
  ...
  async request(args) {
    ...
    if (this.privKey) {
      ...
      const sig = await browser.sign(Buffer.from(this.privKey, "hex"), Buffer.from(keccak("keccak256").update(JSON.stringify(userData)).digest("hex"), "hex"));
      session._user = browser.getPublic(Buffer.from(this.privKey, "hex")).toString("hex");
    ...
    return new Promise((resolve, reject) => {
      this.modal._prompt(this.state.clientId, this.state.whiteLabel, this.state.loginConfig, async chunk => {
        if (chunk.cancel) {
          ...
          try {
            const selectedLoginResponse = await this._selectedLogin(lodash_merge(params, chunk));
  ...
  async encrypt(message, privateKey) {
    ...
    return browser.encrypt(browser.getPublic(Buffer.from(privKey, "hex")), message);
  }
  ...
  async decrypt(ciphertext, privateKey) {
    ...
    return browser.decrypt(Buffer.from(privKey, "hex"), ciphertext);
  }
...
export { LOGIN_PROVIDER as L, OPENLOGIN_NETWORK as O, SUPPORTED_KEY_CURVES as S, UX_MODE as U, _get as _, OpenLogin as a, elliptic_1 as e, getHashQueryParams as g };
